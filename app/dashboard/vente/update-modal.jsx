...
export default function UpdateVenteModal({ open, onOpenChange, vente, selectedProgrammation, handleProgrammationSelect }) {
  ...
  useEffect(() => {
    ...
    setData((prev) => ({
      ...prev,
      programmationId: vente?.programmationId,

      produitId: vente?.produitId || '',
      typeId: vente?.typeId || '',
      typeFactureVenteId: vente?.typeFactureVenteId || '',
      clientCommanderId: vente?.commandeClient?.clientId || '',
      clientId: vente?.clientId || '',
      date: vente?.date?.split("T")?.[0] || '',
      unitePrice: vente?.unitePrice || 0,
      qteTotal: vente?.qteTotal || 0,
      remise: vente?.remise || 0,
      transport: vente?.transport || 0,
    ...
    // le client
    setClient(clients?.find((cl) => cl.id == vente?.clientId))
  ...
  const handleChange = (e) => {
    ...
    if (name == "qteTotal" && value > selectedProgrammation?.resteAvendre) {

      toast.warning(`La quantité maximale est ${selectedProgrammation?.resteAvendre}`)
      return
    }
    setData((prev) => ({
      ...prev,
  ...
  const updateVenteForm = async (e) => {
    ...
    try {
      await toast.promise(
        axiosInstance.put(apiRoutes.updateVente(vente?.id), data),
        {
          loading: `Modification de la vente ${vente?.code} ...`,
          success: async (res) => {
            ...
            return `Vente modifiée avec succès!`
          },
          error: (err) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="md:max-w-[800px] sm:max-w-[480px] overflow-y-auto max-h-[90vh]">
        <DialogHeader className="bg-light p-1">
          <DialogTitle>
            <PencilLine />Modification de la vente
            <span className="badge mx-1 bg-dark rounded border text-white"> {vente?.code}</span>
          </DialogTitle>
          <DialogDescription>
            Remplissez les informations pour modifier cette vente.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={updateVenteForm}>
          ...
          <div className="row">
            ...
            <div className="col-md-6">
              <div className="mb-2">
                <Label htmlFor="qteTotal">Quantité totale <span className="text-danger">*</span>  </Label>
                <Input id="qteTotal"
                  type="number"
                  name="qteTotal"
                  placeholder="Ex: 20"
                  required
                  min={1}
                  max={selectedProgrammation?.resteAvendre}
                  value={data.qteTotal}
                  onChange={handleChange} />
