...
export default function UpdateProgrammationModal({ open, onOpenChange, programmation, handleBonSelect }) {
  ...
  useEffect(() => {
    ...
    setData((prev) => ({
      ...prev,
      commandeId: programmation.commandeId,
      zoneId: programmation.zoneId || '',
      camionId: programmation.camionId || '',
      chauffeurId: programmation.chauffeurId || '',
      avaliseurId: programmation.avaliseurId || '',
      dateProgrammation: programmation.dateProgrammation.split("T")?.[0] || '',
      qteProgrammer: programmation.qteProgrammer || '',
      observation: programmation.observation || ''
    }))
  ...
  const handleChange = (e) => {
    ...
    setData((prev) => ({
      ...prev,
      commandeId: programmation?.commande?.id,
      [name]: type === "file"
        ? files?.[0] ?? null
        : type === "checkbox"
          ? checked
  ...
  const submitUpdateForm = async (e) => {
    ...
    try {
      await toast.promise(
        axiosInstance.put(apiRoutes.updateProgrammation(programmation?.id), data),
        {
          loading: `Modification de la programmation ${programmation?.code} ...`,
          success: async (res) => {
            ...
            return `Programmation modifiée avec succès!`
          },
          error: (err) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="md:max-w-[800px] sm:max-w-[480px] overflow-y-auto max-h-[90vh]">
        <DialogHeader className="bg-light p-1">
          <DialogTitle>
            <PencilLine />Modification de la programmation
