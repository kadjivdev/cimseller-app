...
export default function ValidApprovisionnementModal({ open, onOpenChange, approvisionnement, setReload }) {
    ...
    const submitValidForm = (e) => {
        e.preventDefault()
        toast.promise(
            () => axiosInstance.post(apiRoutes.validateApprovisionnement(approvisionnement.id, {
                validationComment: data.validationComment
            })),
            {
                loading: `Validation de l'approvisionnement ${approvisionnement?.code}  en cours ...`,
                success: (res) => {
                    ...
                    // fermeture du modal
                    onOpenChange(false)
                    return 'Approvisionnement validé avec succès!'
                },
                error: (err) =>{
                    console.log("Erreure de validation de l'approvisionnement :", err)
                ...
                <DialogContent className="sm:max-w-sm text-center">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            Cet approvisionnement <span className="badge bg-light border rounded text-dark"> {approvisionnement?.code}</span> sera validé définitivement.
                        </DialogDescription>
                    </DialogHeader>
