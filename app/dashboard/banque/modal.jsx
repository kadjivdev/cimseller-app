...
export default function UpdateBanqueModal({ open, onOpenChange, banque, setReload }) {
  ...
  useEffect(() => {
    if (!banque) return
    setData({ name: banque.name, description: banque.description || '' })
  }, [banque])
  ...
  const submitUpdateForm = async (e) => {
    ...
    try {
      await toast.promise(
        axiosInstance.put(apiRoutes.updateBanque(banque.id), data),
        {
          loading: `Mise à jour en cours de la banque ${banque?.name}...`,
          success: async (res) => {
